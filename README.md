...
| После Import ничего не изменилось | Нужен именно **Import**, не новый Export. Импортируйте файл `*_EDITED*`. Если снова нажать Export — правки затрутся |
| Сохранил, а в игре старое | В Bitburner: Options → **Import save** → файл с рабочего стола `..._EDITED.json.gz` |
| OneDrive / облако | Лучше копировать сейв в обычную папку (например `C:\Games\`) без синхронизации |
